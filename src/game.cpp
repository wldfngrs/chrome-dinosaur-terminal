...
int init() {
	...
	game_wnd = newwin(screen_area.height() - 2,
			  screen_area.width() - 2,
			  screen_area.top() + 1,
			  screen_area.left() + 1);
...
int run(int* max_score) {
	...
	while (1) {
		...
		if (player.crouched) { player.bounds = {{player.crouch_pos[0].x, player.crouch_pos[0].y}, {player.crouch_pos[c_length-1].x,player.crouch_pos[c_length-1].y}}; }
		else { player.bounds = {{player.pos[0].x, player.pos[0].y}, {player.pos[p_length-1].x,player.pos[p_length-1].y}}; }
		...
		if (feet_tick == 17) {
			feet_tick = 0;
			if (!player.up) playerFeetAnimation(game_wnd, &player);		
		}
		...
		if (game_over) {
			*max_score = player.score > *max_score ? player.score : *max_score;
			...
			mvwprintw(game_wnd, 12, 29, "press any key to retry");							
			wrefresh(game_wnd);
			sleep(0.5); /* Gotta let the gamer read the info */
			flushinp();
